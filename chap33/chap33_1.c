...
void insertAfter(Node* prev_node, int new_data) {
    ...
    // 새 노드의 prev는 원래 prev_node 주소 그 자체를 세팅
...
void deleteNode(Node** head, Node* target_node) {
    ...
    // 경우 2: 삭제할 노드가 마지막 노드가 아닌 경우
    if (target_node->next != NULL) {
        target_node->next->prev = target_node->prev;
    }

    // 경우 3: 삭제할 노드가 첫노드가 아닌 경우
...
int main(void) {
    ...
    // 데이터, 이전 노드 주소, 다음 노드 주소 저장
    // 양방향 탐색 가능, 효율적인 삭제(O(1))
    // 노드당 메모리 사용량 증가(prev 포인터), 더 복잡한 구현
