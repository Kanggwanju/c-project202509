...
// 맨 앞에 새 데이터를 추가하는 로직
// 헤드는 노드 포인터
// 헤드 내부의 값을 바꾸기 위해서 노드 포인터의 주소를 받기 -> 이중 포인터
// 포인터의 주소를 받기 위해서 함수에서는 이중 포인터를 사용
...
void freeList(Node** head) {
    ...
    while(current != NULL) {
        next_node = current->next; // 해제할 대상의 다음번 노드를 백업
        free(current); // 현재 노드 해제
        current = next_node; // 다음번 반복을 위해 대상 노드 수정
    }
...
int main(void) {
    ...
    // 연결리스트 전체 구성 예시
    // header 노드         100번지 노드            200번지 노드
    //   100번지    ->     값 1   200번지   ->    값2    null
    // header(감시 노드)가 null 값일 경우 빈 리스트
    // 마지막 노드의 포인터가 null 값일 경우 리스트의 끝

    // 유연한 삽입, 삭제, 크기 제한 없음
    // 느린 탐색 속도: 특정 원소 찾으려면 무조건 head부터 순차 탐색(O(N))
    // 
    ...
    insertAtEnd(&head, 9777);
    insertAtEnd(&head, 9889);
